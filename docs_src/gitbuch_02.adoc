...
image::index.svg[id="fig.index",scaledwidth="90%",width="90%"]
...
<<fig.sha>> shows how it works, where we calculate the checksum of `hello.py`.
...
.SHA-1 Algorithm
image::sha.svg[id="fig.sha",scaledwidth="90%",width="90%"]
...
<<fig.objekte>> shows three objects from the example project -- a blob, a tree, and a commit.{fn25}
...
.Git Objects
// .Git-Objekte
image::git-objects.svg[id="fig.objekte",scaledwidth="90%",width="90%"]
...
You will first see the four commits that make up the Git repository, including the `e2c67eb` commit shown in <<fig.objekte>>.
...
The hierarchical relationship of the objects listed above is shown in <<fig.objekte-zusammenhang>>.
...
image::git-objects-hierarchy.svg[id="fig.objekte-zusammenhang",scaledwidth="65%",width="65%"]
...
As you can see, all the information in <<fig.objekte>> is output: the SHA-1 sums of the commit, tree, and direct ancestor, plus the author and committer (including the date as a Unix timestamp), and the commit description.
...
As in <<fig.objekte>> the tree referenced by the commit contains one blob for each of the two files, and one tree (also: _subtree_) for the `test` directory.
...
The four commits that make up the sample repository are shown again in <<fig.struktur>>, but in a different way: The dashed bordered tree and blob objects indicate unchanged objects, all others were added or changed in the corresponding commit.
...
.Repository Content
image::repository-content.svg[id="fig.struktur",scaledwidth="80%",width="80%"]
...
An example graph is shown in <<fig.graph>>.
...
// .Ein Commit-Graph
.A Commit Graph
image::commit-graph.svg[id="fig.graph",scaledwidth="45%",width="45%"]
...
<<fig.graph-mit-refs>> shows the same commit graph with the `master`, `HEAD`, `feature`, and `bugfix` branches.
...
image::commit-graph-with-refs.svg[id="fig.graph-mit-refs",scaledwidth="75%",width="75%"]
