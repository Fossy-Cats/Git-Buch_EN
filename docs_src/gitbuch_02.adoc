...
`--patch` bzw. kurz `-p`.footnote:[Genaugenommen führt die Option
  `-p` direkt in den _Patch-Mode_ des
  _Interactive-Mode_ von `git add`.  Der Interactive-Mode
...
`e` (_edit_):: Editiere den aktuellen Hunk.footnote:[Git öffnet dann
...
"`hinzuzufügen`".footnote:[Tatsächlich erstellt Git einen neuen Commit, dessen Änderungen eine Kombination der Änderungen des alten Commits und des Index ist. Der neue Commit _ersetzt_ dann den alten.]  Außerdem können Sie die
...
World!__-Programm, diesmal in der Programmiersprache Python.footnote:[Sie
...
Inhalt{empty}footnote:[Git speichert sämtliche Objekte
  ...
  _Loose Objects_ und _Packfiles_. Die "`losen`"
  ...
  Datei). Im Gegensatz dazu sind Packfiles komprimierte _Archive_
...
auflöst.footnote:[Intern kennt
  ...
  erkennen und diese platzsparend zu _Packfiles_
  zusammenzuschnüren.]
...
bewegen.footnote:[Diese beiden Eigenschaften
  _gerichtet_ und _azyklisch_ sind die einzig notwendige
