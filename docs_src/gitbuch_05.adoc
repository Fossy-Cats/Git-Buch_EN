...
landen.footnote:[Der Stern (`*`) wird
  wie bei der Shell auch als _Wildcard_ interpretiert und zieht
...
verwenden,footnote:[Im Git-Jargon werden
  solche Remotes als _Anonymous_ bezeichnet.] zum Beispiel, um die
...
die Patches geschickt werden sollen.footnote:[Sofern auf Ihrem System kein 'Mail
  ...
  the SMTP server__ der bereits erwähnten Man-Page beschriebenen
  Einstellungen an.]
...
Beispiel
_dpaste_{empty}footnote:[http://dpaste.com/]
oder
_gist.github_{empty}footnote:[https://gist.github.com/]
...
zu untersuchen und zu verändern.footnote:[Die `libgit.a` wird beim
  ...
  _reentrant_ oder Thread-sicher, so dass sie nur eingeschränkt
...
unterhalb von `libgit2` übertragen.footnote:[Achten Sie daher darauf, dass Sie mit
  ...
  _dürfen_. Je nach Lizenz ist die Benutzung einer Software möglicherweise erlaubt,
  aber nicht die Weiterverbreitung (engl. _Distribution_).]
