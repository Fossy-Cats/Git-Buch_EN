...
While it is possible to take a centralized approach with GitHub (see <<fig.zentraler-workflow>>) by giving others access to your own repositories, the most common form of sharing is an Integration Manager workflow (see <<fig.developer-public-workflow>>).
