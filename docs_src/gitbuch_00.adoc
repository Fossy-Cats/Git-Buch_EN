...
<<chap.praxis>> discusses practical aspects of version control.
...
Only <<sec.distributed-git>> looks at the distributed aspects of Git: how to share changes between repositories, how developers can collaborate.
Then <<sec.workflows>> provides an overview of strategies for coordinating development work in a project.
...
To find your way around the shell faster and better, we strongly recommend adding git functionality to the shell, such as displaying the branch in the prompt (see <<sec.shell-integration>>).
...
The installation of Git is described in detail in <<sec.installation>>.
