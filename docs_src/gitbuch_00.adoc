...
ist Dreh- und Angelpunkt der Entwicklung: Dort werden Patches eingeschickt, Neuerungen diskutiert und auch Fragen zur Benutzung beantwortet.
...
The Git mailing list [2] is the lynchpin of the development: patches are sent in, new features are discussed, and questions about using Git are answered.
...
Umsteigern aus dem Subversion-Umfeld ist der 'Git-SVN Crash
  Course'{empty}{fn6}
zu empfehlen, eine Gegenüberstellung von Git- und Subversion-Kommandos, mit der Sie Ihr Subversion-Wissen in die Git-Welt übertragen.
////
