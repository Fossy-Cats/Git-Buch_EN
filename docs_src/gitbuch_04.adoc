...
`git add` hinzu und lassen Sie den Prozess per `git
  ...
  Fall das Feature _Reuse Recorded Resolution_ ansehen, kurz
  _rerere_. _Rerere_ speichert Konfliktlösungen und
...
_TopGit_{empty}footnote:[Den Quellcode finden Sie unter
...
Alternative zu TopGit ist _Stacked Git_{empty}footnote:[Kurz `stg` oder StGit, erreichbar unter
...
`--preserve-merges`.footnote:[Das funktioniert auch problemlos,
  sofern alle Abzweigungen und Zusammenführungen _oberhalb_ der
...
_nach_ dem der Fehler auftritt.footnote:[Dieser Commit muss natürlich
