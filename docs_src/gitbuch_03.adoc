...
_direkten_ Vorfahren, `<ref>^2` den zweiten usw.footnote:[Aufgrund der
Tatsache, dass bei einem Merge die Reihenfolge der direkten Vorfahren
gespeichert wird, ist es wichtig, immer vom kleineren _in_ den
...
vorhanden.footnote:[Wie lange sie dort verweilen, bestimmen Sie
  mit entsprechenden Einstellungen für die _Garbage Collection_
...
Version leicht ersichtlich ist.footnote:[Eine
  ...
  Kapitel 6 des Buches _Open Source Projektmanagement_ von Michael
...
`recursive`:: Dies ist die Standard-Strategie, die
    ...
    haben,footnote:[Die
      ...
      _Topologie_ der Commits (d.h. die Anordnung, wo
...
_KDiff3_{empty}footnote:[http://kdiff3.sourceforge.net/]
und
_Meld_{empty}footnote:[http://meld.sourceforge.net/].
...
Lösung gefunden wurde.footnote:[Die Meldung `Automatic
  ...
  _nicht_ durch einen 3-Wege-Merge gelöst werden konnte. Da
...
_AsciiDoc_{empty}footnote:[AsciiDoc ist eine simple,
...
Änderungen per Cherry-Pick ausgetauscht haben, zu Konflikten kommen.footnote:[Das liegt daran, dass das
  ...
  werden drei Trees verglichen, in denen _unter anderen_ diese
...
aufzuheben.]  Die Notation `^A` bedeutet also: _nicht_ die Commits,
...
_ein_ Objekt, das dann angezeigt wird.footnote:[Dies ist nicht nötigerweise ein Commit -- das können auch Tags oder Blobs sein.]
...
da sein.footnote:[Ob die Commits nicht schon aufgrund
  ...
  Sie eine _Garbage-Collection_ per `git gc` durchführen.
  Siehe auch <<sec.gc>>.
]
...
dieser Commit referenziert.footnote:[Wollen
  ...
  stattdessen `git log --since=_two weeks ago_`.]
